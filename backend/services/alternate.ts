...
export class Alternate implements ISearchSubService {
	...
	async searchProducts(
		...
		const maxProducts = parseInt(
			document?.getElementsByClassName("shown-products-count-bottom")?.[0]?.parentElement?.lastChild.textContent.trim().split(" ")[1] ||
				"24"
		);

		if (maxProducts > this.pageSize)
			document = new DOMParser().parseFromString(
				await (await fetch(this.getUrl(searchTerm, categoryIdentifier) + `&lpf=${Math.floor(maxProducts / this.pageSize) + 1}`)).text(),
				"text/html"
			);
		...
		if (productBoxes?.length) {
			for (const productBox of productBoxes) {
				...
				if (!text.innerText.toLowerCase().includes(searchTerm.toLowerCase()) || text.innerText.toLowerCase().includes("retoure")) continue;
				...
				product.rating = productBox.getElementsByClassName("ratingstars")?.[0]?.getElementsByClassName("fas").length || null;
				...
				product.price =
					parseFloat(productBox.getElementsByClassName("price")?.[0].innerText.trim().split(" ")[1].replace(".", "")) + 15 || null;
